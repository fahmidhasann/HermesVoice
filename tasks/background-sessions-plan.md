...
- [x] **Phase 5 — UX surfacing of background activity** — §5, traps §4.8 *(done 2026-06-08)*
  - [x] `SessionManager` publishes "any session streaming" flag + one-shot `didFinish(id)`
  - [x] Animated menu-bar status item reflects background activity; in-panel pill stays foreground-only
  - [ ] Verify: panel hidden + background stream → menu-bar shows activity; finish posts a cue *(pending live-gateway run)*
  - _Note: `SessionManager` is now an `ObservableObject`. It exposes `@Published private(set) var isAnyStreaming` (maintained in `streamingDidBegin/End` via a `defer { isAnyStreaming = activity.count > 0 }`, so it tracks the same ref-count that drives the App Nap assertion — true while ANY foreground/background session streams) and `let didFinish = PassthroughSubject<String, Never>()`. `ChatSession` gained a one-shot `var onFinished: ((String) -> Void)?` fired in `finishAssistant` **only when a non-empty assistant message is committed** (per §4.8 — read at finish time, NOT inferred from `state == .done`, which evaporates after the 1.5s done→idle window); `SessionManager.register` wires it to `didFinish.send(id)` alongside the existing begin/end callbacks, so a backgrounded session still reports its finish. The `manager` stays private to the `OverlayViewModel` facade, which re-publishes the two signals as `anyStreamingPublisher`/`sessionFinishedPublisher` (`AnyPublisher`s). `AppDelegate.subscribeToBackgroundActivity()` (called from `applicationDidFinishLaunching`) sinks both into `activityCancellables`: `setStatusItemStreaming(_:)` runs a gentle repeating opacity pulse on the status-item button (a `Timer` animating `button.alphaValue` 1.0↔0.35 — AppKit's `NSView.addSymbolEffect` is NOT available under the CLT toolchain, so a manual alpha pulse is used instead; idempotent, restores alpha 1.0 on stop), and `postBackgroundFinishCue(for:)` does a one-shot dim→restore flash **only when the finish was in the background** (panel hidden OR finished id ≠ foreground id) AND no streaming pulse is already running. The in-panel status pill is untouched: `OverlayView` reads `viewModel.state` (the foreground mirror), so it stays foreground-only by construction (§4.8). The status *menu* still only rebuilds on open (`menuNeedsUpdate`), which is exactly why activity is surfaced via the animated status-item image rather than a menu line. `swift build -c release` OK; `swift run HermesVoiceTests` → 252 checks, 0 failures (unchanged — manager/facade/AppKit layer is not CLT-unit-testable); `./build-app.sh` OK (build 25); `graphify update .` done. Manual verification (panel hidden + background stream → menu-bar pulses; background finish → flash cue) still pending a live gateway run._
